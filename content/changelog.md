...
- `ClusterTemplateInstance` controller now correctly updates the `ClusterTemplateInstance` custom resource status and the namespace count upon the deletion of a namespace.
- Conflict between `ClusterTemplateInstance` controller and `kube-contoller-manager` over mentioning of secret names in `secrets` or `imagePullSecrets` field in `ServiceAccounts` has been fixed by temporarily ignoring updates to or from `ServiceAccounts`.
...
- `ClusterTemplateInstance` controller now ensures that its underlying resources are force-synced when a namespace is created or deleted.
...
- The `ClusterTemplateInstance` reconcile flow has been refined to process only the namespace for which the event was received, streamlining resource creation/deletion and improving overall efficiency.
...
- enhance: ClusterTemplateInstance CRs now actively watch the resources they apply, and perform functions to make sure they are in sync with the state mentioned in their respective Templates

> More information about ClusterTemplateInstance's sync at [Sync Resources Deployed by ClusterTemplateInstance](./kubernetes-resources/template/how-to-guides/resource-sync-by-tgi.md)
...
- fix: ClusterTemplateInstance now cleans up leftover Template resources from namespaces that are no longer part of TGI namespace selector
...
- feat: Namespace controller will now trigger ClusterTemplateInstance when a new matching namespace is created
...
- fix: Enhancement of ClusterTemplateInstance Namespace event listener

### v0.3.27

- feat: ClusterTemplateInstance will create resources instantly whenever a Namespace with matching labels is created

### v0.3.26

- fix: Update reconciliation frequency of ClusterTemplateInstance

### v0.3.25

- feat: ClusterTemplateInstance will now directly create template resources instead of creating TemplateInstances
...
- To migrate to Tenant-Operator:v0.3.25 perform the following steps
    ...
    - Delete TemplateInstances created by ClusterTemplateInstance (Naming convention of TemplateInstance created by ClusterTemplateInstance is `group-{Template.Name}`)
    - Update version of Tenant-Operator to v0.3.25 and set the replicas count to 2. After Tenant-Operator pods are up ClusterTemplateInstance will create the missing resources
...
- feat: Add validation webhook for TemplateInstance & ClusterTemplateInstance to prevent their creation in case the Template they reference does not exist

### v0.3.14

- feat: Added Validation Webhook for Quota to prevent its deletion when a reference to it exists in any Tenant
- feat: Added Validation Webhook for Template to prevent its deletion when a reference to it exists in any Tenant, ClusterTemplateInstance or TemplateInstance
